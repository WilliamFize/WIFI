...
void HardwordOpenMic(u8 sender_ch, u8 led_change)
{
	if(mic.status != MIC_STATUS_OPEN)
	{
		...
		WifiCoreCommandSend(0x56, &mic.send_ch, 1);
	}
	if(led_change)
...
void HardwordCloseMic(u8 send_to_host, u8 led_change)
{
	switch(mic.status)
	{
		case MIC_STATUS_OPEN:
			...
			if(send_to_host)
				HostCommandSend(6, 0x00, 0x00, 0, 0x00);
			WifiCoreCommandSend(0x56, &mic.status, 1);
		...
		case MIC_STATUS_WAIT:
			mic.status = MIC_STATUS_CLOSE;
			if(send_to_host)
				HostCommandSend(8, 0x00, 0x00, 0, 0x00);
			break;
		default:
			break;
	}
