...
u16 DataFlashWriteNumber = 0; 
u16 DataFlashReadNumber  = 0;

void DataFlashClean_NS(void)
{
	u8 i;
	for(i = 0; i <= 7; i++)
		FMC_Erase((0x0001F000UL+(i)*0x200));
}
...
void DataFlashFindDataNumber_NS(u16 DataCount)
{
	...
	while(BeginNumber <= EndNumber)
	{
		...
		if(FMC_Read((0x0001F000UL+(HalfNumber)*4)) == 0xffffffffUL)
		{
			if(HalfNumber == BeginNumber)
			{
				DataFlashClean_NS();
				DataFlashWriteNumber = 0;
				DataFlashReadNumber  = 0;
				break;
			}
			else if(FMC_Read((0x0001F000UL+(HalfNumber - 1)*4)) != 0xffffffffUL)
			{
		...
		{
			if(HalfNumber == EndNumber)
			{
				DataFlashReadNumber  = HalfNumber + 1 - DataCount;
				DataFlashWriteNumber = 1023 + 1;
				break;
			}
			else if(FMC_Read((0x0001F000UL+(HalfNumber + 1)*4)) == 0xffffffffUL)
			{
...
void DataFlashRead_NS(u32 *SaveData,u16 DataCount)
{
	...
	{
		SaveData[Save_number] = FMC_Read((0x0001F000UL+(DataFlashReadNumber + Save_number)*4));
	}
}
...
void DataFlashSave(u32 *SaveData,u16 DataCount)
{
	...
	if(DataFlashWriteNumber + DataCount - 1 > 1023)
	{
		DataFlashClean_NS();
		DataFlashReadNumber  = 0;
		DataFlashWriteNumber = 0;
	...
	for(Save_number = 0; Save_number < DataCount; Save_number++)
		FMC_Write((0x0001F000UL+(Save_number + DataFlashWriteNumber)*4), SaveData[Save_number]);
