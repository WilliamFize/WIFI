...
void DwinUSARTInit(void)
{
	...
	((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->GPC_MFP &= ~((1UL<<4) | (1UL<<5));
    ((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->GPC_MFP |= ((1UL<<4) | (1UL<<5));
	((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->ALT_MFP &= ~((1UL<<29) | (1UL<<30));
    ((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->ALT_MFP |= (1UL<<29)| (1UL<<30);
...
{
    u8 u8InChar = 0xFF;
    u32 u32IntSts = ((UART_T *) ((( uint32_t)0x40000000) + 0x50000))->ISR;
    ...
    if(u32IntSts & (1ul << 8))
    {
        while((((((UART_T *) ((( uint32_t)0x40000000) + 0x50000)))->ISR & (1ul << 0))>>0))
        {
            ...
            switch(dwin_recv.find_step)
			{
				case 0x00:
							if(u8InChar == 0x5A)
								dwin_recv.find_step = 0x01;
							break;
				case 0x01:
							if(u8InChar == 0xA5)
								dwin_recv.find_step = 0x02;
							else
								dwin_recv.find_step = 0x00;
							break;
				case 0x02:
							...
							{
								dwin_recv.q[dwin_recv.w_p].len = u8InChar;
								dwin_recv.revcnt = 0;
								dwin_recv.find_step = 0x03;
							}
							break;
				case 0x03:
							dwin_recv.q[dwin_recv.w_p].data[dwin_recv.revcnt++] = u8InChar;
							if(dwin_recv.revcnt == dwin_recv.q[dwin_recv.w_p].len)
							{
								dwin_recv.w_p = (dwin_recv.w_p + 1) & (16 - 1);
								dwin_recv.find_step = 0x00;
							}
							break;
...
void DWINWriteByte(u8 * data, u8 len)
{
...
void DwinSendRegValue(u8 reg, u8 val)
{
	u8 data[6] = {0x5A, 0xA5, 0x03, 0x80};
	data[4] = reg;
	data[5] = val;
	DWINWriteByte(data, 6);
}

void DwinSendResValue  (u16 id, u16 val)
{
	u8 data[8] = {0x5A, 0xA5, 0x05, 0x82};
	data[4] = (id>>8) & 0xFF;
	data[5] = id & 0xFF;
	data[6] = (val>>8) & 0xFF;
	data[7] = val & 0xFF;
	DWINWriteByte(data, 8);
}

u16 dwin_cur_id;
void DwinJumpTo(u16 dlgId)
{
	u8 data[7] = {0x5A, 0xA5, 0x04, 0x80, 0x03};
	dwin_cur_id = dlgId;
	data[5] = (dlgId >> 8) & 0xFF;
	data[6] = dlgId & 0xFF;
	DWINWriteByte(data, 7);
}

void DwinSetChar(u16 tbId, u8 c)
{
	u8 data[7] = {0x5A, 0xA5, 0x04, 0x82};
	data[4] = (tbId >> 8) & 0xFF;
	data[5] = tbId & 0xFF;
	data[6] = c;
	DWINWriteByte(data, 7);
...
void DwinSetString(u16 tbId, u8 * str, u8 len)
{
	u8 head[6] = {0x5A, 0xA5, 0x04, 0x82};
	
	head[2] = len + 3 + 1;
	head[4] = (tbId >> 8) & 0xFF;
	head[5] = tbId & 0xFF;
	
	DWINWriteByte(head, 6);
	DWINWriteByte(str,  len);
	DWINWriteByte("\0", 1);
}
