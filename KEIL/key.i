...
{
	{2UL,		3,    	1,	KEY_POWER_SW_VALUE},
	{3UL,	3, 	0,	KEY_MIC_VALUE},
	{3UL,		2,		0,	KEY_VIP_VALUE},
};
...
static void keyInputGet(void)
{
	...
	for(i = 0; i < KEY_NULL_VALUE; i++)
	{
		if(info.master_unit)
		{
			if((*((volatile uint32_t *)(((((( uint32_t)0x50000000) + 0x4000) + 0x0200)+(0x40*(key_info[i]. port))) + ((key_info[i]. pin)<<2)))) == key_info[i].level)
			{
	...
	if(value != last_value) {
		...
		if(key_press.delay < 0xffff) key_press.delay++;
	}
}
			...
			else if(key_press.delay == 30)
			{
				...
				switch(key_press.value)
				{
					case KEY_MIC_VALUE: 
						switch(info.unit_opration)
						{
							case UNIT_OPRATION_SETID_MODE:
								HostCommandSend(45, setid.current >> 8, setid.current, wifi.mac, 6);
								break;
					...
					case KEY_VIP_VALUE:
						...
						{
							(*((volatile uint32_t *)(((((( uint32_t)0x50000000) + 0x4000) + 0x0200)+(0x40*(5UL))) + ((2)<<2)))) = 0;
							HostCommandSend(9, 0x00, 0x00, 0, 0);
						}
						break;
					case KEY_POWER_SW_VALUE:
						if(popup.enable)
