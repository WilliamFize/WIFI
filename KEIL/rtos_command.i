...
typedef enum
{
	RTOS_STARTUP_WAIT_UART,
	RTOS_STARTUP_WAIT_AP,
	RTOS_STARTUP_WAIT_HOST,
	RTOS_STARTUP_FINISH,
	
	RTOS_STARTUP_UART_ERROR,
	RTOS_STARTUP_NET_CARD_ERROR,
	RTOS_STARTUP_AP_DISCONNECT,
}RTOS_STARTUP_E;
...
void RtosUSARTInit(void);
void RtosCommandSend(u8 cmd, u8 *data, u8 len);
u16  RtosRecvPolling(void);
...
typedef struct
{
	rtos_recv_queue_t recv_queue[100];
	u8 revcnt;
	u8 find_step;
	u8 write;
	u8 read;
}rtos_recv_packet_t;
static rtos_recv_packet_t rtos_recv_packet;
...
void RtosUSARTInit(void)
{
	...
	rtos_recv_packet.find_step = 0x00;
	rtos_recv_packet.read = 0;
	rtos_recv_packet.write = 0;
...
void UART1_IRQHandler(void)
{
    ...
    if(u32IntSts & (1ul << 8))
    {
        while((((((UART_T *) ((( uint32_t)0x40100000) + 0x50000)))->ISR & (1ul << 0))>>0))
        {
            ...
            switch(rtos_recv_packet.find_step)
			{
				case 0x00:
							if(u8InChar == 0x5A)
								rtos_recv_packet.find_step = 0x01;
							break;
				case 0x01:
							if(u8InChar == 0xA5)
								rtos_recv_packet.find_step = 0x02;
							else
								rtos_recv_packet.find_step = 0x00;
							break;
				case 0x02:
							...
							{
								rtos_recv_packet.recv_queue[rtos_recv_packet.write].len = u8InChar;
								rtos_recv_packet.revcnt = 0;
								rtos_recv_packet.find_step = 0x03;
							}
							break;
				case 0x03:
							if(rtos_recv_packet.revcnt < rtos_recv_packet.recv_queue[rtos_recv_packet.write].len)
							{
								rtos_recv_packet.recv_queue[rtos_recv_packet.write].data[rtos_recv_packet.revcnt++] = u8InChar;
							}
							else
							{
								rtos_recv_packet.find_step = 0x00;
								if(u8InChar == 0xFC)
								{
									rtos_recv_packet.write = (rtos_recv_packet.write + 1) % 100;
								}
							}
							break;
			}
...
void RtosWriteByte(u8 * data, u8 len)
{
	u8  i = 0;
...
void HostCommandSend(u8 cmd, u8 para1, u8 para2, u8 *data, u8 len)
{
	...
	RtosWriteByte(head, 7);

	if(data != 0)
	{
		RtosWriteByte(data, len);
	}

	RtosWriteByte(&tail, 1);
}
...
static void HostRecvCheck(u8 *data, u8 len)
{
	...
	switch(cmd)
	{
		...
		case 2:
			...
			{
				info.host_connect 	= 1;
				info.rtos_startup	= RTOS_STARTUP_FINISH;
...
u16 RtosRecvPolling(void)
{
	...
	if(rtos_recv_packet.read != rtos_recv_packet.write)
	{
		type         = rtos_recv_packet.recv_queue[rtos_recv_packet.read].data[0];
		eff_data     = rtos_recv_packet.recv_queue[rtos_recv_packet.read].data + 1;
		eff_data_len = rtos_recv_packet.recv_queue[rtos_recv_packet.read].len  - 1;
		...
		switch(type)
		{
			case 0x01:
				...
				wifi.connect 		= 1;
				wifi.rssi	 		= 4;
				info.rtos_startup	= RTOS_STARTUP_WAIT_HOST;
				info.overcount 		= 0;
				break;
			case 0x02:
				...
				memcpy(info.rtos_ver, eff_data, 8);
		...
		rtos_recv_packet.read = (rtos_recv_packet.read + 1) % 100;
