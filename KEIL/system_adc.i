...
void AdcNopRuning(void)
{
	...
	for(i = 0; i < 50; i++)
	{
		...
		while(((((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000)))->ADSR & (1ul << 3) ? 1 : 0));
	}
	meet_unit_adc.get_value_ok = 1;
...
void SystemAdcInit(void)
{
	...
	((((GPIO_T *) (((( uint32_t)0x50000000) + 0x4000) + 0x00C0)))->OFFD |= ((0x33)<<16));
	...
	((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->GPD_MFP  &= ~((1UL<<0)   | (1UL<<1)   | (1UL<<4)   | (1UL<<5));
    ((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->GPD_MFP  |=   (1UL<<0)  | (1UL<<1)  | (1UL<<4)  | (1UL<<5);
    ((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->ALT_MFP1 &= ~((1UL<<16)  | (1UL<<17)  | (1UL<<20)  | (1UL<<21));
    ((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->ALT_MFP1 |=   (1UL<<16) | (1UL<<17) | (1UL<<20) | (1UL<<21);

	SYS_LockReg();

	((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000))->ADCR    = (((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000))->ADCR & (~(3ul << 2))) | (2UL<<2);
	((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000))->ADCR   |= (1ul << 1) | (1ul << 0);
	
	
	((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000))->ADCHER |= 0x33;
...
{
	meet_unit_adc.adc0_value[meet_unit_adc.sample_count] = ((((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000)))->ADDR[(0)] & (0x3FFul << 0));
	meet_unit_adc.adc1_value = ((((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000)))->ADDR[(1)] & (0x3FFul << 0));
	meet_unit_adc.adc4_value = ((((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000)))->ADDR[(4)] & (0x3FFul << 0));
	meet_unit_adc.adc5_value = ((((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000)))->ADDR[(5)] & (0x3FFul << 0));
	if(meet_unit_adc.sample_count < 6)
...
static const u16 battery_rank_list[5][2] = 
{
	{0x0000,                       0x01D3},
	{0x01DA - 0x000A, 0x0207 + 0x000A},
	{0x0207 - 0x000A, 0x0234 + 0x000A},
	{0x0234 - 0x000A, 0x0262 + 0x000A},
	{0x0262 - 0x000A, 0xFFFF}
};
...
void SystemReadAdcPolling(void)
{
	...
	if(meet_unit_adc.sample_count == 6)
	{
		...
		level = AdcBubbleSort(meet_unit_adc.adc0_value, 6);
		
#line 214 "..\\user\\system_adc.c"
		
		u8_temp = info.battery;
		Battery_conver(level, &u8_temp);

		usb_state = (meet_unit_adc.adc5_value >= 0x140) ? 1 : 0;
		...
		if(info.battery == 0)
		{
			{DisplaySetPopup(POPUP_NULL); OLEDDMAFill(128, 48, 0, 2); {(info . lang) ? (OLEDDMAShowStr("Battery Too Low!", 0, 4)) : (OLEDDMAShowPic(image_bat_too_low, 128, 16, 0, 4));};};
	...
	if(meet_unit_adc.get_value_ok == 1)
	{
		...
		if(mic.use && mic.mode == MIC_MODE_VOICE)
		{
			if(meet_unit_adc.adc1_value >= mic.v_sen)
			{
				...
				{
					HostCommandSend(4, info.master_unit, 0x00, 0, 0x00);
					mic_recheck_count  = 150;
				}
		...
		else if(meet_unit_adc.adc4_value >= 0x03EA)
		{
			Wm8974RegisterInit();
			Wm8974SpecialParmInit();
			wm8974_recheck_count = 100;
		}
		
		meet_unit_adc.get_value_ok = 0;
		((((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000)))->ADCR |= (1ul << 11));
	}
}
