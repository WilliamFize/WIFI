...
void WifiCoreUSARTInit(void)
{
    ...
    ((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->GPB_MFP  |= ((1UL<<4) | (1UL<<5));
	((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->ALT_MFP  &= ~((1UL<<15) | (1UL<<18));
    ...
    if(u32IntSts & (1ul << 8))
    {
        while((((((UART_T *) ((( uint32_t)0x40100000) + 0x50000)))->ISR & (1ul << 0))>>0))
        {
            ...
            switch(wifi_core_recv.find_step)
			{
				case 0x00:
							if(u8InChar == 0x5A)
								wifi_core_recv.find_step = 0x01;
							break;
				case 0x01:
							if(u8InChar == 0xA5)
								wifi_core_recv.find_step = 0x02;
							else
								wifi_core_recv.find_step = 0x00;
							break;
				case 0x02:
				...
				case 0x03:
							...
							else
							{
								...
								if(u8InChar == 0xFC)
								{
									wifi_core_recv.w_p = (wifi_core_recv.w_p + 1) & (16 - 1);
								}
							}
							break;
			}
...
void HostCommandSendEnterSystem(u8 in)
{
	...
	data[2] = info.battery;
	data[3] = wifi.rssi;
	WifiCoreCommandSend(0xAB, data, 4);
}
	...
	if(info.host_connect)
	{
		switch(cmd)
		{
			...
			case 5:
				switch(para1)
				{
					case 0x00:
						...
						{
							if(popup.enable)
								{DisplaySetPopup(POPUP_MIC_STATUS); OLEDDMAFill(128, 16, 0, 2); {(info . lang) ? (OLEDDMAShowStr("   MIC IS ON    ", 0, 4)) : (OLEDDMAShowPic(image_mic_open, 128, 16, 0, 4));}; OLEDDMAFill(128, 16, 0, 6);};
						}
						break;
					case 0x01:
					case 0x04:
						if(popup.enable)
						{
							{DisplaySetPopup(POPUP_MIC_STATUS); OLEDDMAFill(128, 16, 0, 2); {(info . lang) ? (OLEDDMAShowStr("   MIC IS FULL  ", 0, 4)) : (OLEDDMAShowPic(image_mic_full, 128, 16, 0, 4));}; OLEDDMAFill(128, 16, 0, 6);};
							info.MICFULLFlag=3;
						}
			...
			case 14:
				if(para1 == 0)
				{
					if(info.fire_warning == 0)
					{
						...
						{DisplaySetPopup(POPUP_NULL); OLEDDMAFill(128, 48, 0, 2); {(info . lang) ? (OLEDDMAShowStr("  Fire Warning! ", 0, 4)) : (OLEDDMAShowPic(image_fire_warning, 128, 16, 0, 4));};};
					}
				}
				else
				{
			...
			case 52:
				...
				WifiCoreCommandSend(0x57, detail, 2);
				OLEDDMAShowNumFill(setid . local - 0x3000, 3, 104, 4);;
				OLEDDMAShowNumFill(setid.local - 0x3000, 3, 56,  0);
				break;
			case 3:
				info.unit_opration	= UNIT_OPRATION_MEETING_MODE;
				mic.mode    		= para1;
			...
			case 43:
				if(popup.enable)
				{
					if(para1 == 0x01)
					{
						{DisplaySetPopup(POPUP_MESSGAE); OLEDDMAFill(128, 16, 0, 2); {(info . lang) ? (OLEDDMAShowStr("Plz go to the door of boardroom!", 0, 4)) : (OLEDDMAShowPic(image_go_to_door, 128, 32, 0, 4));};};
					}
					else
					{
						{DisplaySetPopup(POPUP_MESSGAE); OLEDDMAFill(128, 16, 0, 2); {(info . lang) ? (OLEDDMAShowStr("Please go to the console!       ", 0, 4)) : (OLEDDMAShowPic(image_go_to_console, 128, 32, 0, 4));};};
					}
				}
				break;
			case 60:
			...
			case 36:
				switch(para1) {
					case 0x00:
						...
						break;
					case 0x01:
						HostCommandSend(37, 0x00, mic.sen_preset, 0, 0x00);
						break;
					case 0x02:
			...
			case 38:
				switch(para1) {
					...
					case 0x01:
						...
						HostCommandSend(39, 0x00, 0x00, detail, 10);
						break;
					case 0x02:
			...
			case 64:
				...
				HostCommandSend(65, 0x00, 0x00, detail, 2);		
				break;
...
u16 WifiCoreRecvPolling(void)
{
	...
	if(wifi_core_recv.r_p != wifi_core_recv.w_p)
	{
		...
		switch(type)
		{
			...
			case 0x03:		
				...
				if(wifi.rssi != u8_temp)
				{
					...
					{
						HostDisconnectDo();
						{DisplaySetPopup(POPUP_NULL); OLEDDMAFill(128, 48, 0, 2); {(info . lang) ? (OLEDDMAShowStr("  RSSI Too Low! ", 0, 4)) : (OLEDDMAShowPic(image_rssi_too_low, 128, 16, 0, 4));};};
			...
			case 0x06:
				...
				dec[0] = setid.local >> 8;
				dec[1] = setid.local;
				WifiCoreCommandSend(0x57, dec, 2);
				break;
			case 0x07:
		...
		wifi_core_recv.r_p = (wifi_core_recv.r_p + 1) & (16 - 1);
